...
$entries = JevEntries::find()->joinWith('jev0')->where(['accounting_code'=>10101020])->andWhere(['>=','date(date_posted)',$datefrom])->andWhere(['<=','date(date_posted)',$dateto])->all();

$balance = JevEntries::find()->joinWith('jev0')->where(['accounting_code'=>10101020,'type'=>'debit'])->andWhere(['<','date(date_posted)',$datefrom])->sum('amount')-JevEntries::find()->joinWith('jev0')->where(['accounting_code'=>10101020,'type'=>'credit'])->andWhere(['<','date(date_posted)',$datefrom])->sum('amount');
        ...
        <table id="exportTable" width="100%" border="0" style="border-bottom: 1px solid black;">
            <tr>
                <td class="center" colspan="12" style="border-left: 0px;border-right: 0px;padding: 0px;">
                    ...
                    <h3 style="margin:0px;">Petty Cash Journal</h3><br><br>
                </td>
            </tr>
            <tr>
                <th>Date</th>
                <th>Particulars</th>
                <th>Reference</th>
                <th>CASH IN<br>Receipt/Replenishment</th>
                <th>CASH OUT<br>Disbursement</th>
                <th>Balance</th>
                <th>JEV#</th>
                <th>Remarks</th>
            </tr>
            ...
            <?php
                foreach ($entries as $key) {
                    ...
                    if($key->jev0->source!=null){
                        
                    }
                    switch ($key->type) {
                        case 'debit':
                            $model = PcvTracking::find()->where(['jev'=>$key->jev])->one();
                            if($model){
                                echo '<td>PCV-'.$model->pcv_number.'</td>';
                            }else{
                                echo '<td></td>';
                            }
